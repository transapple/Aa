    ...
    return (
      <SafeAreaView style={styles.safe}>
        ...
        <View style={styles.setupSummary}>
          <Text style={styles.setupSummaryTitle}>Academic setup status</Text>
          <Text style={styles.setupSummarySub}>Your core school structure is ready to manage.</Text>
          <View style={styles.setupStats}>
            {SECTIONS.map((section) => (
              <View key={section.id} style={styles.setupStat}>
                <Text style={styles.setupValue}>{data[section.id].length}</Text>
                <Text style={styles.setupLabel}>{section.label}</Text>
              </View>
            ))}
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  setupSummary: { marginHorizontal: spacing.lg, marginTop: -8, marginBottom: 14, backgroundColor: colors.surface, borderRadius: radius.lg, padding: 15, ...shadow.soft },
  setupSummaryTitle: { fontSize: 15, fontWeight: '800', color: colors.ink },
  setupSummarySub: { fontSize: 12, color: colors.inkFaint, marginTop: 3, fontWeight: '500' },
  setupStats: { flexDirection: 'row', justifyContent: 'space-between', marginTop: 14 },
  setupStat: { flex: 1 },
  setupValue: { fontSize: 19, fontWeight: '800', color: colors.teal },
  setupLabel: { fontSize: 10.5, color: colors.inkFaint, marginTop: 2, fontWeight: '600' },
