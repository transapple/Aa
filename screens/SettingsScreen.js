        ...
        <View style={[styles.body, { paddingHorizontal: spacing.lg }]}>
          <Reveal index={0} delay={50}>
            <Card style={styles.profileCard}>
              <View style={styles.profileIcon}><Ionicons name="school" size={21} color={colors.primary} /></View>
              <View style={{ flex: 1 }}>
                <Text style={styles.profileTitle}>{name}</Text>
                <Text style={styles.profileSub}>{motto} · {district}</Text>
              </View>
              <View style={styles.statusPill}><Text style={styles.statusText}>Active</Text></View>
            </Card>
          </Reveal>
          <Text style={styles.sectionLabel}>Manage settings</Text>
...
const styles = StyleSheet.create({
  ...
  profileCard: { flexDirection: 'row', alignItems: 'center', marginBottom: 16, padding: 15 },
  profileIcon: { width: 44, height: 44, borderRadius: 13, backgroundColor: colors.infoBg, alignItems: 'center', justifyContent: 'center', marginRight: 12 },
  profileTitle: { fontSize: 15, fontWeight: '800', color: colors.ink },
  profileSub: { fontSize: 12, color: colors.inkFaint, marginTop: 3, fontWeight: '500' },
  statusPill: { backgroundColor: colors.successBg, paddingHorizontal: 9, paddingVertical: 5, borderRadius: radius.pill },
  statusText: { color: '#0F7A50', fontSize: 10.5, fontWeight: '800' },
